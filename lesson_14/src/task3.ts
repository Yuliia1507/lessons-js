...
function getTotalSum(dataList: DataList[]) {

	const totalSum = dataList.reduce((prevSum, product) => prevSum + product.price, 0)
	return totalSum
}

const totalProductPrice = getTotalSum(dataList)

document.write(`Загальну вартість: ${totalProductPrice}<br>`)


// 2)Знайти кількість товарів, у яких ціна зменшилась(price < old_price).

function getProductsWithLowerPrice(dataList: DataList[]) {
	const prod = dataList.filter(product => product.price < product.old_price).length
	return prod
}

const lowerPrice = getProductsWithLowerPrice(dataList)

document.write(`Кількість товарів, у яких ціна зменшилась: ${lowerPrice}<br>`)


// 3) Товари, які доступні(sell_status: "available")

function getAvailableProd(dataList: DataList[]): DataList[] {
	const available = dataList.filter(prod => prod.sell_status === "available")

	return available
}

const available = getAvailableProd(dataList);
available.forEach(prod => {
	document.write(`ID: ${prod.id},  Sell status: ${prod.sell_status}<br>`)
})
...
function getAvailableProductsInfo(available: DataList[]) {
	const availableProductsInfo = available.map(prod => ({
		id: prod.id,
		price: prod.price,
		old_price: prod.old_price,
		usd_price: (prod.price / 40).toFixed(2)
	}))

	return availableProductsInfo
}

const availableProducts = getAvailableProductsInfo(available)

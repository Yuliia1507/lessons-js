...
function getTotalPrice(websites) {
...
function getWebsitesCountBetweenYears(websites, start, end) {
...
function getWebsitesCountSponsor(websites) {
...
document.write(`3) кількість сайтів, де сума спонсорських вкладень була більшою за 100000: ${websitesCount}<br>`);
// 4) створити загальний список усіх спонсорів(поки можуть повторюватись, просто зібрати усі у масив)
function getSponsorsList(websites) {
    const sponsorsList = [];
...
document.write(`4) створити загальний список усіх спонсорів: ${arrList
    .map(sponsor => `${sponsor.firstName} ${sponsor.lastName}`)
    .join(", ")}<br>`);
// 5) знайти рік, коли прибуток був найбільшим
function getYearOfMaxInvestment(websites) {
...
function sortWebsitesByProfit(websites) {
    return websites.slice().sort((a, b) => b.sponsors.reduce((sum, s) => sum + s.investment, 0) -
...
function splitWebsitesByPrice(websites) {
    const cheap = websites.filter(site => site.price <= 10000).map(site => (Object.assign({}, site)));
    const expensive = websites.filter(site => site.price > 10000).map(site => (Object.assign({}, site)));
